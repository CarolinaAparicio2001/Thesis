...
class LightningModule(_LightningModule):
    ...
    def predict_hidden_state(self, batch, *args, hidden_state_dict, metric_dict, **kwargs):
        ...
        self.hidden_states["error"].append((metric_dict["preds"] - metric_dict["target"]).abs().cpu())
        ...
        self.hidden_states["smoe_weights"].extend(
            [{k: v[i] for k, v in hidden_state_dict["smoe_weights"].items()} for i in range(len(batch["idx"]))]
        )
    ...
    def on_predict_epoch_end(self) -> None:
        ...
        os.makedirs(os.path.join(self.predict_path, "hidden_state"), exist_ok=True)
        pickle.dump(self.hidden_states, open(os.path.join(self.predict_path, "hidden_state", "all.pkl"), "wb"))
        ...
        pickle.dump(self.hidden_states, open(os.path.join(self.predict_path, "hidden_state", "top10.pkl"), "wb"))
        ...
        data.to_csv(os.path.join(self.predict_path, "hidden_state", "top10.csv"), index=False)

        smoe_weight_output_path = os.path.join(self.predict_path, "hidden_state", "smoe_weights")
        os.makedirs(smoe_weight_output_path, exist_ok=True)
        moe_weight_output_path = os.path.join(self.predict_path, "hidden_state", "moe_weights")
        os.makedirs(moe_weight_output_path, exist_ok=True)

        def plot_moe_piror(data, labels, output_path):
            plt.bar_label(plt.bar(range(len(data)), data, tick_label=labels), label_type="edge")
        ...
        for i, smoe_weights in enumerate(self.hidden_states["smoe_weights"]):
            keys = ["summarization", "drugs", "diseases", "criteria", "smiless_transformer_concat", "description"]
            smoe_weights = {k: smoe_weights[k] for k in keys if k in smoe_weights}
            if "smiless_transformer_concat" in smoe_weights:
                smoe_weights["smiless"] = smoe_weights.pop("smiless_transformer_concat")
            plot_smoe_weights(smoe_weights, os.path.join(smoe_weight_output_path, f"{i}.png"))
