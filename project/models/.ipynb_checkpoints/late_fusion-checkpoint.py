...
class LateFusion(nn.Module):
    def __init__(
        ...
        self.smiles_transformer_encoder = {
            k: nn.ModuleList(
                [
            ...
            for k in [
                f"smiles_transformer{p}" for p in ["", "_concat", "_summarization"]
            ]
            if k in self.input_parts
        }

        if self.smiles_transformer_encoder:
    ...
    def forward(self, data):
        ...
        for key in [f"smiles_transformer_{p}" for p in ["concat", "summarization"]]:
            if key in self.input_parts:
                ...
                feature = self.smiles_transformer_encoder[key][1](
        ...
        for key in ["table", "summarization"] + [
            f"{k}_{p}"
            for k in ["smiles", "drugs", "diseases"]
            ...
            embedding, attention_mask = self.add_embedding(
        ...
        for key in ["smiles", "description", "drugs", "diseases"]:
            if key not in self.input_parts:
                continue
            embedding_index += 1
        ...
        preds = {
            k: self.sigmoid(self.fc(v)).squeeze(-1)
