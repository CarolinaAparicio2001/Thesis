# mmcto.py with ADMET integration
import math
import torch
...
class MMCTO(nn.Module):
    def __init__(
        ...
        scalar_inputs = [
            "enrollment", "admet_absorption", "admet_distribution",
            "admet_metabolism", "admet_excretion", "admet_toxicity"
        ]
        self.scalar_proj = nn.ModuleDict({
            name: nn.Linear(1, model_dim) for name in scalar_inputs if name in self.input_parts
        })
    ...
    def forward(self, data):
        ...
        for key in self.input_parts:
            if key not in data or key in features or key.startswith("smiles_transformer") or key == "criteria":
                continue
            embedding_index += 1
