# ctod.py with ADMET integration and safe collate_fn
import json
import os
import ast
...
class CTODataset(BaseDataset):
    def __init__(self, data_prefix=None, ann_file_name=None, augment=False, input_parts=None, **kwargs):
        ...
        if input_parts is None:
            input_parts = [
                "table", "summarization", "description", "criteria", "enrollment"
            ] + [f"{k}{p}" for k in ["smiles", "smiles_transformer", "drugs", "diseases"] for p in ["", "_concat", "_summarization"]] \
              + [f"admet_{k}" for k in ["absorption", "distribution", "metabolism", "excretion", "toxicity"]]
    ...
    def collate_fn(batch):
        def collate(batch):
            ...
            for name in ["enrollment"] + [f"admet_{k}" for k in ["absorption", "distribution", "metabolism", "excretion", "toxicity"]]:
                if all(name in b for b in batch):
                    res[name] = torch.stack([b[name] for b in batch], dim=0).unsqueeze(1)
    ...
    def load_data_list(self):
        ...
        split = self.ann_file_name.split("_")[-1]
        admet_path = self.data_prefix.get("admet_path", self.data_prefix["data_path"])
        admet_data = load_admet_all(admet_path, split)

        data_list = []
        for i, row in data.iterrows():
            cur_data = {"idx": i}
            if "label" in row:
                cur_data["label"] = torch.tensor(float(row["label"]), dtype=torch.float)
            ...
            enrollment = row.get("enrollment", None)
            try:
                cur_data["enrollment"] = torch.tensor(float(enrollment), dtype=torch.float) \
                    if pd.notna(enrollment) else torch.tensor(float('nan'), dtype=torch.float)
            except:
                cur_data["enrollment"] = torch.tensor(float('nan'), dtype=torch.float)

            flag = True
            for name in ["smiles", "drugs", "diseases"]:
                if name in row:
                    d = sorted(set(safe_parse_list(row[name])))
                    if not d:
                        flag = False
                        continue
                    ...
                    if name == "smiles":
                        ...
                        cur_data.update(attach_admet_features(d, admet_data))
            ...
            if drug_description:
                cur_data["description"] = [
                    drug_description.get(drug_name, "This is a drug.") for drug_name in safe_parse_list(row.get("drugs", []))
                ]
            ...
            for i in range(len(dataset)):
                ...
                for name in data:
                    if name not in ["label", "sample_idx", "idx", "criteria"]:
                        tokenizer = Tokenizer if "smiles_transformer" not in name else SMILESTokenizer
                        if isinstance(data[name], dict) or isinstance(data[name], list):
                            continue
